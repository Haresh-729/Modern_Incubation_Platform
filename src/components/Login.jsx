            ...
            <div className="mx-4 flex-col justify-center w-auto h-screen, md:mx-4 lg:mx-8 pt-2">
                <div data-aos="fade-left" className=" float-none md:float-right flex flex-col items-center justify-center md:w-1/2 lg:w-2/5 py-4 sm:mx-14 md:mx-4 lg:mx-2 lg:py-5  xl:py-14 my-4 lg:my-8 bg-white/30 rounded-[2rem] lg:rounded-[4rem] xl:rounded-[5rem]  border-2 border-offwhite shadow-lg xl:1/3">
                    ...
                    <div className="flex gap-2 lg:gap-8 ">
                        <button onClick={signInWithGoogle} className="items-center justify-center flex m-5 p-1 xl:p-3 w-auto border-[0.1rem] lg:border-[0.2rem] border-grey border-solid hover:bg-offwhite/30 rounded-[0.5rem] lg:rounded-2xl">
                            <img className="w-5 lg:w-6 xl:w-8 m-1" src="https://i.ibb.co/ccpyr20/google.png" alt="img desc" />
                        ...
                        <button onClick={signInWithGithub} className="items-center justify-center flex  m-5  p-1 xl:p-3 w-auto border-[0.1rem] lg:border-[0.2rem] border-grey border-solid hover:bg-offwhite/30 rounded-[0.5rem] lg:rounded-2xl">
                            <img className="w-5 lg:w-6 xl:w-8 m-1" src="https://i.ibb.co/z8SL7zX/icons8-github-48.png" alt="img desc" />
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col items-center justify-center w-full gap-4  xl:gap-6 lg:mt-6 xl:px-12">
                        <div className="relative w-full px-10" id='form'>
                            <input type="email" name="email" id="email" onChange={event => setValues((prev) => ({ ...prev, email: event.target.value }))} className="border-b-2 shadow-sm border-grey bg-white/0 w-full py-2 focus:outline-none focus:shadow-md focus:border-b-blue focus:border-b-2 transition-colors peer" placeholder='Email' autoComplete="off" required pattern="\S+.*" />
                        </div>
                        <div className="relative w-full px-10">
                            <input type="password" name="password" id="password" onChange={event => setValues((prev) => ({ ...prev, password: event.target.value }))} className="border-b-2 shadow-sm border-grey bg-white/0 w-full py-2 focus:outline-none focus:shadow-md focus:border-b-blue focus:border-b-2 transition-colors peer" autoComplete="off" placeholder='Password' required pattern="\S+.*" />
                        </div>
                        <div className='relative w-full px-12'>
                            <button disabled={submitButtonDisabled} className="w-full p-1 my-4 sm:my-8 lg:my-6 xl:my-8 font-bold lg:extrabold xl:font-extrabold font-poppins text-sm sm:text-sm md:text-sm lg:text-xl xl:text-2xl text-white bg-blue rounded-[0.3rem] hover:bg-lb focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-opacity-50 sm:text-2xl">
                                LOGIN ACCOUNT
                            </button>
                        </div>
                ...
                <div className='h-screen hidden md:block lg:block md:w-1/2 xl:2/3'>
                    ...
                    <img data-aos="fade-right" className="float-left w-[30rem] md:w-[20rem] lg:w-[40rem] xl:w-[45rem] h-[22rem] md:h-[22rem] lg:h-[32rem] xl:h-[38rem] object-bottom " src="https://i.ibb.co/71DDfsV/Mobile-login-cuate-1.png" alt="img desc" />
                </div>
            </div>
        </div >
    )
